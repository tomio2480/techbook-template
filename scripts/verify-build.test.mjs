...
test('verifyConfigUsesMarkdown: https:// の // をコメント開始と誤認せず、URL 末尾の .html を正しく検出する', () => {
  const dir = makeTempRepo();
  writeConfig(dir, ['https://example.com/foo.html', 'src/chapters/toc.html']);
  const result = verifyConfigUsesMarkdown(dir);
  // https:// をコメント開始と誤認すると、行末までが除去されて .html が
  // 見えなくなり、本来検出すべき .html 参照が見逃されてしまう
  assert.equal(result.ok, false);
});
