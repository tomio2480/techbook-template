...
- 文書内リンクの断片は，GitHub が実際に生成するアンカーへ合わせる．
  見出しの先頭に絵文字を置く決まりのため，断片の先頭へハイフンが付く．
  `## 🔧 機能` に対する断片は `#-機能` である．
  異体字セレクタを伴う絵文字では，その 1 文字も断片へ残る．
  `## 🏷️ GitHub 運用` に対する断片は `#️-github-運用` である．
  見た目で判断せず，`gh api --method POST /markdown/raw` の描画から取る．
  ハイフンの無い形はリンク切れであり，`markdownlint` の `MD051` が検出する．
