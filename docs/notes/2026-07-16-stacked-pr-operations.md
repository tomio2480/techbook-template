...
- クローズ済み PR では base 変更（`gh pr edit --base`）と
  再オープン（`gh pr reopen`）がどちらもできない．
  操作の順序を入れ替えても失敗する．
- `--delete-branch` はローカル側の同名ブランチも削除する．
  ローカルから push し直す復旧手段まで同時に失われる．
...
5. 差分整理（後述）を終えたら，`git push origin --delete <旧base名>`
   で一時ブランチを片付ける．
...
- `git rebase --onto origin/main origin/<旧base名>` で，
  子のコミットだけを main へ載せ替える．
- 復元済みブランチを指定すれば，コミット SHA の特定は不要である．
  一時ブランチを削除済みの場合は `<旧base先端sha>` を指定する．
