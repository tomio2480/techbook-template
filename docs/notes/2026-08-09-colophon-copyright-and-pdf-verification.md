# 奥付への著作権表記の追加とPDF目視検証手段の確立

Issue #63（PR #78）で奥付へ著作権表記を追加した際の設計判断・
発見した実装バグ・PDF目視検証手段の確立を記録する．
...
- 単純な実装ではなく，既存の設計判断を再検討するか維持するかの
  判断が必要だったため，Fable 5（Plan エージェント）へ設計を
  一度エスカレーションした．
...
5. **採用**．`table-layout: fixed` と実測の幅（`width: 100mm`）で
   列幅を固定した．内容に依存しない決定論的なレイアウトになり，
   短い既定値は 1 行に収まり，長い値は欠落せず正しく折り返される．
   NBSP は不要になった．
