...
2 パスでビルドする．最後に `scripts/verify-build.mjs` がビルド完了を検証する．
検証に失敗した場合はコマンドが非 0 で終了するため，
`dist/book.pdf` の内容を確認せず配布しないこと．出力はタグ付き PDF である．
詳細は「[アクセシビリティ（タグ付き PDF）](#-アクセシビリティタグ付き-pdf)」を参照．
...
- 塗り足し（タチキリ）．ページを仕上がりより天地左右へ 3 mm 大きく組む．
  ...
  量は `theme.css` の `--bleed` で変える．電子書籍用の出力には施さない．
  表紙・裏表紙の絵柄は塗り足し込みの寸法（B5 なら 188 x 263 mm）で作る．
  電子書籍用では仕上がりへ切り出し，紙入稿用では塗り足しごと出す．
...
量は `theme.css` の `--bleed` を単一の出所として読み，表紙側で書き写さない．
...
対象は行番号付与・ビルド検証・紙入稿の組版計算などの各検査である．
...
タグ付き PDF の読み順は，回り込みでも原稿の記述順が保たれる．図とキャプションが先に，回り込ませた本文が後に読まれる．検証の記録は [回り込みのタグ読み順検証とキャプション可視化の判断](docs/notes/2026-08-28-figure-wrap-reading-order.md) を参照．
...
[図版の様式と配色 要求・要件](docs/spec/diagram-style.md) が定める．
明度段・図中フォント・検査からの除外と，閾値の根拠はそちらを読む．
ここには配色を替えるときに触る場所だけを書く．

補助記載（矢印・注釈線等）も配色に追従する．使ってよい色は
`--palette-diagram-annotation` と明度段パレットに登録した色だけである．
明度段パレットは `scripts/check-diagram-luminance.mjs` の
`DIAGRAM_TIER_COLORS` で定義する．グレースケール印刷でも判別できるよう，
...
図版の文字は，root の `<svg>` の `font-family` に本文と同じゴシック体スタックを指定する．
値は `theme.css` の `--font-gothic` と同じにする．
量記号を明朝の斜体で組むなど，意図して使う別のスタックは
`ALLOWED_EXTRA_FONT_STACKS` へ登録する．登録の無いスタックは違反として報告する．
書体と大きさは `font-family`・`font-size` へ分けて書く．
`font` の短縮記法と `all` の一括指定も違反として報告する．
...
読めない形である．タグ・要素・コメントの閉じ忘れ，終了タグの対応崩れ，
引用符の対応崩れ，コメントの中の `<!--` と `--` が当たる．
...
- 枠の種別を減らしたときは，`print.css` の焼き済み変数も消す．
  検査対象は `theme.css` にある `--<種別>-icon` の宣言から決まる．
  対応する宣言を失った `--*-icon-baked` は，残り物として検出される．
...
`config/themes/techbook/theme.css` の `--bleed` と，組み上がった PDF が
