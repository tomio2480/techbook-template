...
値は `theme.css` の `--font-gothic` と同じにする．`img` で埋め込んだ SVG はページの
...
子要素には原則フォントを書かず，root の値を継承させる．
書く場合はテーマと同じスタックか登録したスタックに限る．検査もその範囲を許す．

書体と大きさは `font-family`・`font-size` へ分けて書く．
`font: 20px Courier` のような短縮記法は違反として報告する．
登録済みのスタックを短縮記法で書いた場合も同じく違反になる．
短縮記法は値の一部を省略できる．書体の部分だけを取り出す解析は誤りやすい．
`style` 属性・`<style>` 要素の宣言に加え，`font` 属性も同じ扱いとする．
root も例外にしない．例外にすると同じ抜け道が root へ残るためである．
