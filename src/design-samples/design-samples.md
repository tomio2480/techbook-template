...
3 種の枠は、配色を基調色（primary）系へ統一する検討中である。色味が同一になるため、種別は SVG 線画アイコン（電球・書類・警告三角）で判別する。その配置 3 案を並置し、いずれも配色は統一済みの状態で示す。アイコンは mask で塗るためパレット差し替えに追従する。フォントを経由しないため、環境差による豆腐化も起きない。なお案 A・B では既存の種別記号（◆・※・！）を残し、案 C のみ記号をアイコンへ置換している。記号の扱いは採用決定時に合わせて確定する。採用決定後に theme.css へ本実装し、本節は削除する。

<p class="sample-label">案 A: 背景透かし（右下に大きめの淡い線画）</p>
...
<p class="sample-label">案 B: 角ワンポイント（タイトル帯の右端に小さく）</p>

<div class="v-primary-tone v-icon-corner">
...
<p class="sample-label">案 C: 帯先頭置換（種別記号 ◆・※・！ をアイコンへ置換）</p>

<div class="v-primary-tone v-icon-label">
